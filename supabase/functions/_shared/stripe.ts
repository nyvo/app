...
export async function createStripeCheckoutSession(params: {
  ...
  return stripeRequest<StripeCheckoutSession>('/checkout/sessions', {
    ...
    // No Stripe Tax: Framio AS is not MVA-registered (below the 50k threshold),
    // and checkout copy promises "Ingen mva. kommer i tillegg." When the company
    // registers for MVA: configure Stripe Tax (origin address + NO registration)
    // in the dashboard FIRST, then re-enable automatic_tax + address collection
    // here — enabling it without dashboard config makes session creation fail.
