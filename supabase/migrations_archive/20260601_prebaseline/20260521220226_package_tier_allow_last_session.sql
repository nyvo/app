-- Allow the prorated package tier to sell the last remaining session.
--
-- Previously the package was hidden once `remaining_sessions <= 1`, on the
-- assumption that drop-in would cover the final session. That's only true
-- when drop-in is actually offered with a price. If the teacher hasn't set a
-- drop-in price (or hasn't enabled drop-in at all), suppressing the package
-- at 1 session left leaves nothing to sell — buyers see "Påmelding stengt"
-- even though `accepts_late_signups = true` and a seat is open.
--
-- Fix: only hide the package when `remaining_sessions = 0` (nothing left to
-- prorate). At 1 session left the package price simply equals the per-week
-- rate, which is the correct behavior whether drop-in is offered or not.
-- When drop-in IS offered, both tiles are shown — minor duplication beats
-- a dead-end checkout.
