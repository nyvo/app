-- Capture Dintero's `payment_product` (e.g. `vipps.vipps`, `payex.creditcard`,
-- `payex.applepay`) on the signup + payment_attempt so the participant drawer
-- can show which payment method was used. Pre-existing rows stay null.

ALTER TABLE public.signups
  ADD COLUMN IF NOT EXISTS payment_product TEXT;

ALTER TABLE public.payment_attempts
  ADD COLUMN IF NOT EXISTS payment_product TEXT;

-- Recreate create_signup_if_available with an additional p_payment_product param.
-- The signature change requires DROP first.
DROP FUNCTION IF EXISTS public.create_signup_if_available(
  UUID, UUID, UUID, TEXT, TEXT, TEXT, NUMERIC, TEXT, TEXT, TEXT, UUID, UUID, TEXT
);
...
CREATE OR REPLACE FUNCTION public.create_signup_if_available(
  ...
  p_payment_product            TEXT DEFAULT NULL
)
RETURNS JSON
LANGUAGE plpgsql
