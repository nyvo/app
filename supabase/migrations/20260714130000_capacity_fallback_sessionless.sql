-- Capacity fallback for courses with no course_sessions rows.
--
-- create_signup_if_available enforces capacity per SESSION: drop-ins check
-- the chosen session's count, packages scan every session inside the package
-- window. A published course with ZERO course_sessions rows therefore had NO
-- capacity enforcement at all — the scan found nothing to fail and the mint
-- proceeded regardless of max_participants. Every UI-created course gets its
-- sessions from save_course_schedule, so this needed a SQL-seeded or
-- API-created course to trigger (found by the 2026-07-14 overnight QA run:
-- two buyers charged for the one seat of a sessionless max_participants=1
-- course), but the money invariant should not depend on how the course row
-- came to exist.
--
-- Fix: when the course has no sessions at all and max_participants is set,
-- fall back to a course-wide confirmed-signup count under the same advisory
-- lock. Exact by construction — drop-ins require a session, so on a
-- sessionless course every signup occupies a full seat. Courses WITH
-- sessions keep the existing per-session semantics, bit for bit.
--
-- The soft pre-payment check in create-stripe-connect-session gets the same
-- fallback (deployed alongside this migration) so buyers are told "Kurset er
-- fullt" before paying instead of at webhook settle time.

CREATE OR REPLACE FUNCTION public.create_signup_if_available(p_seller_id uuid, p_course_id uuid, p_ticket_type_id uuid, p_participant_name text, p_participant_email text, p_participant_phone text, p_amount_paid numeric, p_course_session_id uuid DEFAULT NULL::uuid, p_buyer_id uuid DEFAULT NULL::uuid, p_note text DEFAULT NULL::text, p_payment_product text DEFAULT NULL::text, p_payment_status text DEFAULT 'paid'::text, p_stripe_payment_intent_id text DEFAULT NULL::text, p_platform_fee_nok numeric DEFAULT 0)
 RETURNS json
 LANGUAGE plpgsql
...
-- Same execute surface as before (webhook/service paths only) — restated so
-- the migration is self-contained.
REVOKE ALL ON FUNCTION public.create_signup_if_available(uuid, uuid, uuid, text, text, text, numeric, uuid, uuid, text, text, text, text, numeric) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.create_signup_if_available(uuid, uuid, uuid, text, text, text, numeric, uuid, uuid, text, text, text, text, numeric) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_signup_if_available(uuid, uuid, uuid, text, text, text, numeric, uuid, uuid, text, text, text, text, numeric) TO service_role;
