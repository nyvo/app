-- Restore the session-write guard flag dropped by 20260714100000.
--
-- 20260714100000_course_instructors.sql replaced public.save_course_schedule
-- with a CREATE OR REPLACE copied from the superseded 20260705200000 body —
-- but 20260711130000_course_integrity_guards.sql (lines 128-314) had already
-- replaced that same function, adding one load-bearing line right after
-- `v_is_draft := v_course.status = 'draft';`:
--
--   PERFORM set_config('app.course_sessions_server_write', 'true', true);
--
-- This is the sanctioned, transaction-local bypass flag for the
-- enforce_course_session_delete_guard trigger (see 20260711130000 for the
-- full rationale). Because 20260714100000 has the highest timestamp, it won
-- last and silently dropped that flag from the live function.
--
-- This migration restores the exact 20260711130000 function body (including
-- the set_config call and its comment) and re-applies, on top of it, the two
-- instructor columns that 20260714100000 legitimately added. The only diff
-- against the 20260711130000 source is those two CASE lines in the
-- `UPDATE public.courses SET` block.
