-- create_signup_if_available gains p_ticket_label_override (2026-07-15).
--
-- The honor-system student/pensjonist discount marks the charge on the
-- payment attempt's ticket_label_snapshot ("Hele kurset – student/pensjonist
-- (−20 %)"), but this RPC snapshots the label from the TIER row, so the mark
-- never reached the signup the roster displays. The override lets the Stripe
-- mint paths (webhook + sweep) carry the attempt's charge-time label through;
-- NULL/blank falls back to the tier label, so the free-signup and
-- teacher-add callers are unaffected.
--
-- Signature change (not CREATE OR REPLACE): appending a defaulted parameter
-- would otherwise create an ambiguous overload for named-arg PostgREST calls.
-- Same DROP + CREATE pattern as 20260714140000. Body is otherwise identical
-- to 20260714140000's definition.

DROP FUNCTION public.create_signup_if_available(uuid, uuid, uuid, text, text, text, numeric, uuid, uuid, text, text, text, text, numeric, numeric);

CREATE FUNCTION public.create_signup_if_available(p_seller_id uuid, p_course_id uuid, p_ticket_type_id uuid, p_participant_name text, p_participant_email text, p_participant_phone text, p_amount_paid numeric, p_course_session_id uuid DEFAULT NULL::uuid, p_buyer_id uuid DEFAULT NULL::uuid, p_note text DEFAULT NULL::text, p_payment_product text DEFAULT NULL::text, p_payment_status text DEFAULT 'paid'::text, p_stripe_payment_intent_id text DEFAULT NULL::text, p_platform_fee_nok numeric DEFAULT 0, p_service_fee_nok numeric DEFAULT 0, p_ticket_label_override text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
...
BEGIN
  ...
  INSERT INTO public.signups (
    ...
    p_ticket_type_id,
    COALESCE(NULLIF(BTRIM(p_ticket_label_override), ''), v_tier.label),
    v_tier.audience, v_tier.ticket_kind,
...
REVOKE ALL ON FUNCTION public.create_signup_if_available(uuid, uuid, uuid, text, text, text, numeric, uuid, uuid, text, text, text, text, numeric, numeric, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.create_signup_if_available(uuid, uuid, uuid, text, text, text, numeric, uuid, uuid, text, text, text, text, numeric, numeric, text) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_signup_if_available(uuid, uuid, uuid, text, text, text, numeric, uuid, uuid, text, text, text, text, numeric, numeric, text) TO service_role;
