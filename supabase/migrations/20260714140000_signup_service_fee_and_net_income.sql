-- Seller income = NET (what the studio actually receives), and the data to
-- compute it.
--
-- "Inntekt" summed signups.amount_paid — the BUYER's total including the
-- 5 % tjenestegebyr (which goes to the platform) — so a 2 600 kr course
-- showed 2 730 kr of "income" the studio never receives. Decision 2026-07-14:
-- the income headline shows net, no support line; buyer-paid fees stay where
-- they already live (checkout breakdown + receipt).
--
-- net per signup = amount_paid − service_fee_nok − platform_fee_nok
--   · service_fee_nok: buyer-paid tjenestegebyr (5 %, min 9 / max 149) —
--     pulled back to the platform via the Stripe application fee.
--   · platform_fee_nok: the free-tier ("Start") 5 % platform take, also part
--     of the application fee. 0 on Pro. Already snapshotted on signups.
-- For both tiers the result equals the price the seller set (minus the
-- platform take on Start) — i.e. what Stripe pays out.
--
-- 1. signups.service_fee_nok — snapshot column (platform_fee_nok's sibling).
-- 2. create_signup_if_available gains p_service_fee_nok. The parameter list
--    changes, so the old signature must be DROPPED first (CREATE OR REPLACE
--    with a new signature would create an ambiguous overload for PostgREST).
--    Callers that don't pass it (create-free-signup, older webhook builds)
--    keep working via the DEFAULT 0.
-- 3. Backfill from payment_attempts via the stripe_payment_intent_id
--    backlink (free/teacher-added signups have no attempt → stay 0).
-- 4. seller_income_series subtracts both fees (net_nok now truly net).

ALTER TABLE public.signups
  ADD COLUMN service_fee_nok numeric NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.signups.service_fee_nok IS
  'Buyer-paid tjenestegebyr (NOK) included in amount_paid but pulled back to the platform via the Stripe application fee — snapshotted at mint (sibling of platform_fee_nok). Seller net = amount_paid − service_fee_nok − platform_fee_nok.';

-- 2. Extend the mint RPC ------------------------------------------------------

DROP FUNCTION public.create_signup_if_available(uuid, uuid, uuid, text, text, text, numeric, uuid, uuid, text, text, text, text, numeric);

CREATE FUNCTION public.create_signup_if_available(p_seller_id uuid, p_course_id uuid, p_ticket_type_id uuid, p_participant_name text, p_participant_email text, p_participant_phone text, p_amount_paid numeric, p_course_session_id uuid DEFAULT NULL::uuid, p_buyer_id uuid DEFAULT NULL::uuid, p_note text DEFAULT NULL::text, p_payment_product text DEFAULT NULL::text, p_payment_status text DEFAULT 'paid'::text, p_stripe_payment_intent_id text DEFAULT NULL::text, p_platform_fee_nok numeric DEFAULT 0, p_service_fee_nok numeric DEFAULT 0)
 RETURNS json
 LANGUAGE plpgsql
...
BEGIN
  ...
  ELSE
    IF v_course.max_participants IS NOT NULL THEN
      IF NOT EXISTS (SELECT 1 FROM public.course_sessions WHERE course_id = p_course_id) THEN
        -- Sessionless course: the per-session scan below has nothing to look
        -- at, which previously meant NO capacity enforcement at all. Enforce a
        -- course-wide confirmed count instead — exact here, because drop-ins
        -- require a session, so every signup occupies a full seat.
        ...
        END IF;
      ELSIF v_package_end_date IS NOT NULL THEN
  ...
  INSERT INTO public.signups (
    ...
    payment_product,
    amount_paid, platform_fee_nok, service_fee_nok, created_at, updated_at
  ) VALUES (
    ...
    p_payment_product,
    p_amount_paid, COALESCE(p_platform_fee_nok, 0), COALESCE(p_service_fee_nok, 0), NOW(), NOW()
  )
...
REVOKE ALL ON FUNCTION public.create_signup_if_available(uuid, uuid, uuid, text, text, text, numeric, uuid, uuid, text, text, text, text, numeric, numeric) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.create_signup_if_available(uuid, uuid, uuid, text, text, text, numeric, uuid, uuid, text, text, text, text, numeric, numeric) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_signup_if_available(uuid, uuid, uuid, text, text, text, numeric, uuid, uuid, text, text, text, text, numeric, numeric) TO service_role;

-- 3. Backfill from the payment attempt (paid Stripe signups only) ------------

UPDATE public.signups s
SET service_fee_nok = pa.service_fee_nok
FROM public.payment_attempts pa
WHERE s.stripe_payment_intent_id IS NOT NULL
  AND pa.stripe_payment_intent_id = s.stripe_payment_intent_id
  AND COALESCE(pa.service_fee_nok, 0) > 0;

-- 4. Income series → net -----------------------------------------------------

CREATE OR REPLACE FUNCTION public.seller_income_series(p_seller_id uuid, p_from timestamp with time zone)
...
AS $function$
  SELECT
    ...
    -- Net to the seller: buyer total minus the buyer-paid tjenestegebyr and
    -- the Start-tier platform take (both pulled back via the application
    -- fee) minus refunds. A full refund clamps to 0.
    sum(greatest(0::numeric,
      coalesce(s.amount_paid, 0)
      - coalesce(s.service_fee_nok, 0)
      - coalesce(s.platform_fee_nok, 0)
      - coalesce(s.refund_amount, 0)
    )) AS net_nok
...
REVOKE ALL ON FUNCTION public.seller_income_series(uuid, timestamp with time zone) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.seller_income_series(uuid, timestamp with time zone) FROM anon;
GRANT EXECUTE ON FUNCTION public.seller_income_series(uuid, timestamp with time zone) TO authenticated;
