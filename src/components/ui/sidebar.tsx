...
function SidebarProvider({
  ...
  // Below lg the 16rem sidebar would leave too little room for dashboard
  // content, so compact screens keep the drawer. At lg and above (including
  // iPad Pro landscape), it becomes a persistent, user-collapsible sidebar.
...
function Sidebar({
  ...
  dir,
  ...props
}: Omit<React.ComponentProps<"div">, "onDrag" | "onAnimationEnd"> & {
  side?: "left" | "right"
  ...
  if (collapsible === "none") {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          "flex h-full w-(--sidebar-width) flex-col bg-sidebar text-sidebar-foreground-muted",
          className
        )}
        {...props}
      >
      ...
      >
        <Drawer.Portal>
          ...
          <Drawer.Content
            ...
            data-slot="sidebar"
            data-mobile="true"
            className="fixed inset-y-0 left-0 z-50 h-full w-(--sidebar-width) bg-sidebar p-0 text-sidebar-foreground-muted"
            style={
              {
          ...
          >
            <div className="sr-only">
              <Drawer.Title>Sidemeny</Drawer.Title>
              <Drawer.Description>
                Viser sidemenyen på mobil.
              </Drawer.Description>
            </div>
  ...
  return (
    <div
      className="group peer hidden text-foreground-muted lg:block"
      data-state={state}
    ...
    >
      ...
      <div
        ...
        className={cn(
          "relative w-(--sidebar-width) bg-transparent transition-[width] duration-200 ease-[var(--ease-out)] motion-reduce:transition-none",
      ...
      <div
        ...
        data-side={side}
        className={cn(
          "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-[var(--ease-out)] motion-reduce:transition-none data-[side=left]:left-0 data-[side=left]:group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)] data-[side=right]:right-0 data-[side=right]:group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)] lg:flex",
...
function SidebarTrigger({
  ...
  return (
    ...
    >
      <PanelLeft />
      <span className="sr-only">Vis/skjul sidemeny</span>
    </Button>
  )
}
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      title="Vis/skjul sidemeny"
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 transition-[left,right,transform] ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:start-1/2 after:w-[2px] hover:after:bg-border sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "relative flex min-h-0 w-full flex-1 flex-col bg-canvas lg:peer-data-[variant=inset]:mt-3 lg:peer-data-[variant=inset]:rounded-t-xl lg:peer-data-[variant=inset]:border-l lg:peer-data-[variant=inset]:border-t lg:peer-data-[variant=inset]:border-border",
        className
      )}
      {...props}
    />
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        "flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-foreground-muted outline-hidden transition-[margin,opacity] duration-200 ease-[var(--ease-out)] motion-reduce:transition-none group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 focus-visible:ring-sidebar-ring [&>svg]:size-4 [&>svg]:shrink-0",
        className
      )}
      {...props}
    />
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        showOnHover &&
          "group-focus-within/menu-item:opacity-100 group-hover/menu-item:opacity-100 peer-data-active/menu-button:text-foreground aria-expanded:opacity-100 lg:opacity-0",
        className
      )}
      {...props}
    />
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "ml-7 mt-1 flex min-w-0 flex-col gap-1 border-l border-border pl-2 group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
    />
