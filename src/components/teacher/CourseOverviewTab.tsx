...
export function CourseOverviewTab({
  ...
  // Single published courses have no overview sections of their own (no
  // checklist, kursplan, or series-only toggles) — the participant roster
  // rendered alongside this tab carries that case. Render nothing here so the
  // merged Oversikt collapses cleanly to the roster instead of an empty wrapper.
  const hasContent =
    isWaitingForDintero ||
    status === 'draft' ||
    showKursplanCard ||
