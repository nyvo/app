 ...
 * Android gets a working escape link (`intent:` reopens the page in the real
 * browser); iOS has no sanctioned escape, so it gets the ⋯-menu instruction.
...
export function InAppBrowserNotice({ className }: { className?: string }) {
  ...
  return (
    <div className={cn('rounded-xl bg-info-subtle px-3.5 py-2.5 text-sm leading-relaxed text-info', className)}>
      {metaBrowserLabel()} kan avbryte innloggingen.{' '}
      {escapeUrl ? (
        <a href={escapeUrl} className="font-medium underline underline-offset-2">
          Åpne i nettleser
        </a>
      ) : (
        'Åpne siden i nettleseren din fra ⋯-menyen.'
      )}
    </div>
  )
}
