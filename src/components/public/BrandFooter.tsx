 ...
 * Deliberately static: not a link, no hover — a signature, not a nav exit
 * out of the studio's funnel. Ratified on the join page 2026-07-20,
 * extended to the whole public funnel (storefront → course detail →
 * checkout → receipt).
 *
...
export function BrandFooter({ className }: { className?: string }) {
  return (
    <footer className={cn('mt-auto flex justify-center pb-8 pt-12', className)}>
      <div className="flex select-none items-center gap-2 opacity-60" aria-hidden="true">
        <UpNextLogo size="sm" />
