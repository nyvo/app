...
export const CourseCard = ({
  ...
  const displayDate = getEventDisplayDate(course.next_session, course.start_date);
  const timeInfo = extractTimeFromSchedule(course.time_schedule);

  // Build secondary metadata
  const metaItems: string[] = [];

  // Course series cards include the day name (e.g. "Mandag")
  if (course.course_type === 'course-series') {
        ...
        <div className="flex gap-4">
          ...
          {displayDate ? (
            <div
              ...
              aria-label={formatDateLong(displayDate)}
            >
