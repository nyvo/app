...
interface FloatingFieldProps extends Omit<React.ComponentPropsWithoutRef<'input'>, 'placeholder'> {
  /** Rests as the field's content at 15px; floats to an 11px top-left
    ...
    return (
      <div className="relative">
        <input
          ...
          placeholder=" "
          className={cn(
            'peer h-[52px] w-full min-w-0 rounded-xl border border-input bg-surface px-[15px] pt-[22px] pb-[6px] text-[15px] text-foreground outline-none transition-colors duration-150 ease-out placeholder:text-transparent',
        ...
        <label
          htmlFor={id}
          className={cn(
            'pointer-events-none absolute left-[15px] top-1/2 -translate-y-1/2 text-[15px] text-foreground-muted transition-all duration-150 ease-out',
