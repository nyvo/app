...
- **Inputs:** dashboard forms use a static label above the input. Public pages may use a floating-label treatment on purpose (Apple/Shop aesthetic) — if a public surface already does this, keep it; "shared primitives" does not mean identical input chrome everywhere.
...
2. Route to the closest preview. Many surfaces have an auth-free `/dev/*` preview route (defined in `src/App.tsx`, files in `src/pages/dev/`): e.g. `/dev/token-preview`, `/dev/checkout-rework`, `/dev/checkout-form-rework`, `/dev/detail-rework`, `/dev/dashboard-preview`, `/dev/courses-grid-preview`, `/dev/courses-list-preview`, `/dev/month-grid-preview`, `/dev/payout-preview`, `/dev/income-chart-preview`, `/dev/entity-card-preview`, `/dev/onboarding-preview`, `/dev/create-course-preview`, `/dev/tier-preview`, `/dev/billing-preview`, `/dev/modals-buttons-toasts`. If you're building something new, add or extend a `/dev/*` preview so it's viewable without auth.
...
- **Full Studio design-system spec** (deep token/component/pattern reference): the user must explicitly invoke `studio-design` — don't auto-fire it; this skill is the lightweight pre-build gate.
