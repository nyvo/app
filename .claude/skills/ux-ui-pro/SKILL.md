...
- **Type discipline.** Two faces (ratified 2026-07-22, supersedes all-Geist): Inter (`--font-sans`) for body/labels/controls; Open Sauce One (`--font-display`) for titles — h1–h3 get it via the `@layer base` rule in `src/index.css`, div/span title components via the `font-display` utility. Never add raw `font-family` per page. No `tracking-*` utilities on typed text — per-size letter-spacing is baked into the `--text-*` token. Display caps at 5xl (48px); 6xl+ are intentionally unset and won't compile. Geist Mono stays for code surfaces only (`--font-mono`).
