...
- **Type discipline.** Two faces (ratified 2026-07-22, supersedes all-Geist): Inter (`--font-sans`) for everything except page-level headlines; Open Sauce One (`--font-display`) for the per-screen `h1` (text-2xl+) and the landing's large marketing h2s only — applied via the `@layer base` rule on `h1` in `src/index.css` (landing.css opts its big h2s in). Section/card/dialog titles stay Inter. Never add raw `font-family` or `font-display` per page. No `tracking-*` utilities on typed text — per-size letter-spacing is baked into the `--text-*` token. Display caps at 5xl (48px); 6xl+ are intentionally unset and won't compile. Geist Mono stays for code surfaces only (`--font-mono`).
- **No serif anywhere.** `--font-serif` was retired 2026-07-15 (EB Garamond removed from the bundle); marketing display is tight semibold Open Sauce One.
- **Expression gradient, identical primitives.** Landing = high expression (tight semibold display face, azure accents + colored artwork, dark chrome bands/cards, grain) → storefronts = neutral frame (the teacher's brand is the hero) → checkout = zero expression → dashboard = baseline. The interactive primitives are the same on every surface; only the expression dial moves.
