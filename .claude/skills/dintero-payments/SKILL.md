...
### Splits
Every `order.items[]` that should pay out to a teacher carries `splits: [{ payout_destination_id, amount }]`. The `payout_destination_id` is the teacher's `sellers.dintero_seller_id`. The platform's own cut (the service-fee line) must be split to a **registered** payout destination — per Dintero's Terms for Sellers (Annex 1), the platform "needs to add themselves as a Seller" to receive splits. Do NOT assume an unsplit line/remainder stays with the platform account: that behavior is undocumented, and splits to unknown destination ids don't fail — the payout is silently **postponed** until a matching approved seller exists. (Current code still splits to a literal `'platform'` id; phase 1 of the pricing plan replaces it with the platform's real registered destination.)
