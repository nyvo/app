...
### Splits
Every `order.items[]` that should pay out to a teacher carries `splits: [{ payout_destination_id, amount }]`. The `payout_destination_id` is the teacher's `organizations.dintero_seller_id`. Service fee lines don't carry splits (they stay with the platform account).
